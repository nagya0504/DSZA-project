...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2024. Jan 10. 21:17
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addBook` (IN `user_id` INT, IN `title` VARCHAR(100), IN `author` VARCHAR(100), IN `price` INT, IN `description` TEXT, IN `image` VARCHAR(100))   INSERT INTO `product`
VALUES(NULL, user_id, title, author, price, description, image, 1, NOW(), NULL)$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectAllFav` (IN `userId` INT)   SELECT `favourites`.`id`, `favourites`.`product_id`, `product`.`title`, `product`.`author`, `product`.`price`, `product`.`description`, `product`.`image`
FROM `favourites`
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateBookAuthor` (IN `id` INT, IN `author` VARCHAR(100))   UPDATE `product`
SET `product`.`author` = author
WHERE `product`.`id` = id$$

DROP PROCEDURE IF EXISTS `updateBookDesc`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateBookDesc` (IN `id` INT, IN `description` TEXT)   UPDATE `product`
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateBookPrice` (IN `id` INT, IN `price` INT)   UPDATE `product`
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateBookTitle` (IN `id` INT, IN `title` VARCHAR(100))   UPDATE `product`
...
(1, 1, 'asd', 'asd asd', 1000, 'valami szép könyv', 'img/card-img.png', 1, '2023-09-26 08:19:50', NULL),
(4, 2, 'dddd', 'eeee', 3333, 'abcd', 'img/bej-book.jpg', 4, '2023-09-26 09:04:45', NULL),
(5, 2, 'asdasd', 'asdasdasd', 1234, 'assssssssssaaaaasdddd', 'img/header.png', 1, '2023-12-14 17:05:56', NULL),
(6, 2, 'postman könyv', 'Postman', 10000, 'Egy nagyon szép könyv amit a postmannel töltöttem fel.', 'img/reg-book.jpg', 1, '2023-12-14 17:06:00', NULL),
(7, 2, 'postman könyv 2', 'Postman', 10000, 'Egy nagyon szép könyv amit a postmannel töltöttem fel.', 'img/card-img.png', 1, '2023-12-14 17:35:50', NULL),
(8, 2, 'postman könyv 3', 'Postman', 10000, 'Egy nagyon szép könyv amit a postmannel töltöttem fel.', 'img/upload_book.jpeg', 1, '2023-12-14 17:45:14', NULL),
(9, 2, 'postman könyv 4', 'Postman', 10000, 'Egy nagyon szép könyv amit a postmannel töltöttem fel.', 'img/card-img.png', 1, '2023-12-14 17:55:51', NULL),
(10, 2, 'postman könyv 4', 'Postman', 10000, 'Egy nagyon szép könyv amit a postmannel töltöttem fel.', 'img/card-img.png', 4, '2023-12-14 18:04:26', '2023-12-15 09:04:30'),
(11, 2, 'postman könyv 4', 'Postman', 10000, 'Egy nagyon szép könyv amit a postmannel töltöttem fel.', 'img/card-img.png', 1, '2023-12-14 18:17:49', NULL),
(12, 2, 'mysqlkonyv', 'asd', 7564, 'asd', 'img/card-img.png', 1, '2024-01-07 18:30:06', NULL),
(13, 2, 'postman könyv 0107', 'Postman', 111, 'Egy nagyon szép könyv amit a postmanen keresztül küldve töltöttem fel.', 'img/card-img.png', 1, '2024-01-07 18:38:46', NULL),
(15, 2, 'weblapkönyv', 'weblap', 7795, 'szép könyv a weblapon', 'img/card-img.png', 1, '2024-01-07 18:43:29', NULL),
(17, 3, 'nem a 2es user könyve', 'asd', 4535, 'asd', 'img/card-img.png', 1, '2024-01-08 11:05:44', NULL);
